...
public class HuffmanCoding {
    ...
    public static ArrayList<CharFreq> makeSortedList(String filename) {
        ...
        for (int i=0; i<128; i++){
            if (array[i]!=0){
                list.add(new CharFreq((char) i, (double) array[i]/count));
                count2++;
            }
        }

        // finding index of element needing wrap
        int index = 0;
        if(count2==1){
            for(int i=0; i<128; i++){
                if(array[i]!=0){
            ...
            if(index==127){
                ...
                list.add(new CharFreq((char)(index+1), 0));
            }
        }
    ...
    public static TreeNode makeTree(ArrayList<CharFreq> sortedList) {
        ...
        while((!source.isEmpty()) || (target.size()!=1)){
            for(int i=0; i<2; i++){
                if(target.isEmpty()){
                    temp.enqueue(source.dequeue());
                }
                else if (source.isEmpty()){
                    temp.enqueue(target.dequeue());
                }
                else if (source.peek().getData().getProbOccurrence()==target.peek().getData().getProbOccurrence() || target.peek().getData().getProbOccurrence()>source.peek().getData().getProbOccurrence()){
                    temp.enqueue(source.dequeue());
                }
                else {
                    temp.enqueue(target.dequeue());
            ...
            TreeNode n1 = temp.dequeue();
            TreeNode n2 = temp.dequeue();
            TreeNode node = new TreeNode(new CharFreq(null, (n1.getData().getProbOccurrence()+n2.getData().getProbOccurrence())), n1, n2); 
            target.enqueue(node);
...
private static void traverse(TreeNode root, String encode, String[] array){
    if(root==null){
        return;
    }

    if(root.getLeft()==null && root.getRight()==null){
        array[(int)root.getData().getCharacter()]=encode;
    }

    traverse(root.getLeft(), encode+"0", array);
    traverse(root.getRight(), encode+"1", array);
}
    /**
