...
public class RUMDbSearchEngine {
	...
	public void insertMoviesIntoHashTable ( String inputFile ) {
		...
		for(int i=0; i<input.size(); i++){
			...
			for (int k=1; k<list.size(); k++){
				...
				String word = isWord(list.get(k));
			   
					if(word!=null) {
						Location loc = new Location(title, k);
	...
	public void insertWordLocation (String word, Location loc) {
		if(getWordOccurrence(word)!=null){
			...
			if (hashTable[index]==null){
					...
					WordOccurrence head = hashTable[index];
					insert.next=head;
					hashTable[index]=insert;
				}
			}
	...
	private static MovieSearchResult containsMSR(ArrayList<MovieSearchResult> list, String title){
	...
	public ArrayList<MovieSearchResult> createMovieSearchResult (String wordA, String wordB) {
		...
		ArrayList<MovieSearchResult> list = new ArrayList<MovieSearchResult>();
			...
			while(ptr!=null){
				if(ptr.getWord().equals(wordA)){ 
					for(int j=0; j<ptr.getLocations().size(); j++){
						if(containsMSR(list, ptr.getLocations().get(j).getTitle())==null){
							MovieSearchResult result = new MovieSearchResult(ptr.getLocations().get(j).getTitle());
							list.add(result);
							result.addOccurrenceA(ptr.getLocations().get(j).getPosition());
						}
						else{
							MovieSearchResult result = containsMSR(list, ptr.getLocations().get(j).getTitle());
							result.addOccurrenceA(ptr.getLocations().get(j).getPosition());
						}
					}
				}
					...
					if(ptr.getWord().equals(wordB)){
						for(int k=0; k<ptr.getLocations().size(); k++){
							if(containsMSR(list, ptr.getLocations().get(k).getTitle())==null){
								MovieSearchResult result = new MovieSearchResult(ptr.getLocations().get(k).getTitle());
								list.add(result);
								result.addOccurrenceB(ptr.getLocations().get(k).getPosition());
							}
							else{
								MovieSearchResult result = containsMSR(list, ptr.getLocations().get(k).getTitle());
								result.addOccurrenceB(ptr.getLocations().get(k).getPosition());
							}
						}
					}
		...
		if(list.size()==0){
			...
			int countB = 0;
			for(int i=0; i<list.size(); i++){
				if(list.get(i).getArrayListA().size()>0){
					countA++;
				}
				if(list.get(i).getArrayListB().size()>0){
					countB++;
				}
			}
	...
	public void calculateMinDistance(MovieSearchResult msr){
		...
		if(msr == null || listA.size()==0 || listB.size()==0){
			...
			Integer prevA = listA.get(0);
			Integer prevB = listB.get(0);

			while(listA.size()>count && listB.size()>count2){ 

			distances.add(Math.abs(prevA-prevB));

			if(prevA<prevB){
				count++;
					if(count<listA.size()){
					prevA=listA.get(count);
				}
			}
			else{
				count2++;
					if(count2<listB.size()){
					prevB=listB.get(count2);
					}
				}
			}

			Integer min = distances.get(0);
	...
	public ArrayList<MovieSearchResult> topTenSearch(String wordA, String wordB){
		...
		ArrayList<MovieSearchResult> result2 = new ArrayList<MovieSearchResult>();
